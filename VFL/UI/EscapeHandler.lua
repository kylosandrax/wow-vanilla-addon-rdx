...
function VFL.EscapeTo(f, elide)
	...
	if elide then fi = fi + 1; if(fi > n) then return; end end
	VFL:Debug(1, "EscapeTo(): escaping to level " .. fi .. " of " .. n);
	-- Invoke the escape handlers in stack-appropriate order.
	for i=n,fi,(-1) do 
		VFL:Debug(1, "EscapeTo(): calling esch[" .. i .."]");
		if esch[i] then
			(esch[i])();
		else
			VFL.print("!!! NIL ESCAPE HANDLER AT INDEX " .. i);
		end
		esch[i] = nil;
	end
	table.setn(esch, fi - 1); -- why in the hell do we need to do this...?
	VFL:Debug(1, "EscapeTo(): " .. table.getn(esch) .. " handlers left after EscapeTo.");
